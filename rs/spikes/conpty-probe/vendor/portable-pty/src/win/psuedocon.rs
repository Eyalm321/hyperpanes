...
impl PsuedoCon {
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        // PATCHED (conpty-probe experiment): opt into PSEUDOCONSOLE_PASSTHROUGH_MODE (0x8)
        // via env var so we can A/B-measure whether the flag affects the DECSTBM
        // scroll-region repaint. Not upstream behavior.
        let mut flags = PSUEDOCONSOLE_INHERIT_CURSOR
            | PSEUDOCONSOLE_RESIZE_QUIRK
            | PSEUDOCONSOLE_WIN32_INPUT_MODE;
        if std::env::var_os("PORTABLE_PTY_CONPTY_PASSTHROUGH").is_some() {
            flags |= PSEUDOCONSOLE_PASSTHROUGH_MODE;
            eprintln!("[portable-pty PATCHED] passing PSEUDOCONSOLE_PASSTHROUGH_MODE (0x8)");
        }
        let result = unsafe {
